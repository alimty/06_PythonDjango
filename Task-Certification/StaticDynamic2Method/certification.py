...
class CertficationsTable(tables.Table):
    ...
    actions = tables.TemplateColumn('''
        ...
        {% if record.status == 'failed' %}
            <a href="{% url 'admin-panel-certifications-status-succeeded' record.id %}"
            class="confirm-modal mdl-button mdl-js-button mdl-button--icon" title="Force status succeeded">
